...
from ticl.distributions import (
...
class MulticlassSteps:
    ...
    def __call__(self, x):
        ...
        class_boundary_indices = torch.randint(
            0,
            x.shape[0],
            ((self.num_classes - 1) * (self.num_steps - 1) + 1,),
            device=x.device,
        )
        class_boundaries_sorted, _ = x[class_boundary_indices].sort(axis=0)
        step_assignments = torch.searchsorted(
            class_boundaries_sorted.T.contiguous(), x.T.contiguous()
        ).T
        class_assignments = torch.randint(
            0,
            self.num_classes,
            (step_assignments.max() + 1, x.shape[1]),
            device=x.device,
        )
...
class ClassificationAdapter:
        ...
        if self.h["max_num_classes"] == 0:
            ...
            if self.h["num_classes"] > 1 and not self.h["balanced"]:
                if self.h["multiclass_type"] == "rank":
                    ...
                    self.class_assigner = MulticlassSteps(
                        self.h["num_classes"], self.h["multiclass_max_steps"]
                    )
                else:
    ...
    def drop_for_reason(self, x, v):
        nan_prob_sampler = CategoricalActivation(
            ordered_p=0.0, categorical_p=1.0, num_classes_sampler=lambda: 20
        )
        ...
        x[
            d
            < torch.rand((1, d.shape[1], d.shape[2]), device=x.device)
            * 20
    ...
    def __call__(
        ...
        if self.h["num_features_sampler"] == "uniform":
            ...
            raise ValueError(
                f"Unknown num_features_sampler: {self.h['num_features_sampler']}"
            )
        args = {
            "device": device,
            "n_samples": n_samples,
        ...
        if (
            self.h["nan_prob_no_reason"] + self.h["nan_prob_a_reason"] > 0
            ...
            if random.random() < self.h["nan_prob_no_reason"]:  # Missing for no reason
                x = self.drop_for_no_reason(
                    x, get_nan_value(self.h["set_value_to_nan"])
                )
        ...
        info["num_features_used"] = num_features_used
        ...
        if self.h["pad_zeros"]:
            ...
            x = torch.cat(
                [
                    x,
                    torch.zeros(
                        (x.shape[0], x.shape[1], num_features - num_features_used),
                        device=device,
                    ),
                ],
                -1,
...
class ClassificationAdapterPrior:
    ...
    def get_batch(
        ...
        with torch.no_grad():
            ...
            x, y, y_, info = ClassificationAdapter(self.base_prior, self.config)(
